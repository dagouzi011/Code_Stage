...
for i in eachindex(T)
    ...
    # ========= expr1 =========
    term1_expr1 = -c * γ - c * kR
    term2_expr1 = (c - 1) * (c + p - 1) * d2vP
    term3_expr1 = (c - 1) * dvP
    inner_expr1 = term1_expr1 + term2_expr1 + term3_expr1
    prefactor_expr1 = c^2 * kR^2 * (c + p - 1)
    expr1 = λ1 * prefactor_expr1 * inner_expr1

    # ========= expr2 =========
    term1_expr2 = -γ
    term2_expr2 = (c + p - 1) * d2vP
    term3_expr2 = dvP
    inner_expr2 = term1_expr2 + term2_expr2 + term3_expr2
    prefactor_expr2 = c^2 * kR^2 * (c + p - 1)
    expr2 = prefactor_expr2 * inner_expr2

    # ========= 差值 =========
    expr_diff = expr1 - expr2

    # ========= 输出 =========
    println("t = $(round(t, digits=2)) │ λ = $(round.(λ_val, digits=4)) │ c = $(round(c, digits=4)) │ p = $(round(p, digits=4)) │ u = $(round(u_val, digits=4))")
    println("    expr1 = $(round(expr1, digits=6))")
    println("    expr2 = $(round(expr2, digits=6))")
    println("    expr1 - expr2 = $(round(expr_diff, digits=6))")
end
